...
export default function StudentPerformance() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const memberships = await base44.entities.ClassMembership.filter({ teacher_id: me.id, student_id: studentId, status: 'approved' });
      setMember(memberships[0] || null);
      const visible = await base44.entities.Essay.filter({ teacher_ids: me.id, status: 'completed' }, '-created_date', 100);
      setEssays(visible.filter(e => e.created_by_id === studentId));
    } catch {
      setLoadError(true);
    }
  };
