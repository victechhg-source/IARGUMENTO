...
// Casamento ROBUSTO em 3 estratégias encadeadas:
//   1) substring alfanumérica normalizada (tolera pontuação/aspas/caixa/acentos);
//   2) casamento exato por sequência contígua de tokens (igual à versão anterior);
//   3) fuzzy: caminhada gulosa permitindo gaps de até 2 tokens (≥70% de acertos),
//      para quando o modelo adiciona/remove uma palavra pequena no excerpt.
function buildAnnotatedText(transcription: string, stages: any[]): string {
  ...
  const transTokens: Array<{ start: number; end: number; norm: string }> = [];
  {
    const re = /[\p{L}\p{N}]+/gu;
  ...
  const excerptAlnum = (excerpt: string) => {
    let out = '';
    for (const ch of excerpt) if (/[\p{L}\p{N}]/u.test(ch)) out += norm(ch);
    return out;
  };
  ...
  const findOccurrence = (excerpt: string): { start: number; end: number } | null => {
    ...
    // 3) Fuzzy: caminhada gulosa com gaps de até 2 tokens; aceita se ≥70% dos tokens casarem.
    const maxGap = 2;
    for (let startIdx = 0; startIdx < transTokens.length; startIdx++) {
      if (transTokens[startIdx].norm !== exTokens[0]) continue;
      const matchedPositions: number[] = [startIdx];
      let ti = startIdx + 1;
      let ei = 1;
...
Retorne apenas o JSON.`;
    ...
    // IDs determinísticos e únicos por competência — garante que grifo e card
    // "Ver no texto" sempre apontem para a mesma observação (mesmo que o modelo
    // reaproveite ids genéricos como "f1" entre competências diferentes).
