...
Deno.serve(async (req) => {
  try {
    ...
    const { code, account_type } = await req.json();
    ...
    const finalAccountType = account_type === 'student' ? 'student' : 'teacher';
    const updated = await base44.auth.updateMe({ account_type: finalAccountType, school_id: school.id, school_name: school.name });
